...
const Day = ({day, rowIdx}) => {
  ...
  const{setShowEventModal, setDaySelected, savedEvents, setSelectedEvent} = useContext(GlobalContext);
    ...
    <div className='border border-gray-200 flex flex-col'>
      ...
      <div className="flex-1 cursor-pointer" onClick={() => {
        ...
        {dayEvents.map((evt, idx) => (
          <div onClick={() => setSelectedEvent(evt)} className={`bg-${evt.label}-200 p-1 mr-3 text-gray-600 text-sm rounded mb-1 truncate`} key={idx}>
            {evt.title}
          </div>
        ))}
      </div>
